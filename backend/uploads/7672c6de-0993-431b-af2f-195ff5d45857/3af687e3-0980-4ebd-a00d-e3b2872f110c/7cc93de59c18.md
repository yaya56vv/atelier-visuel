...
**État global du code :** Stable — Application fonctionnelle complète (12 étapes + remédiation)

### 4.1 Ce qui fonctionne actuellement

- **Frontend React + Vite + TypeScript** : application complète avec Canvas2D, composants UI, stores, API client
- **Backend FastAPI** : serveur complet avec CRUD, services IA, indexation, seed automatique
...
- **lancer.bat** : lancement en un clic (backend + frontend)
...
- **Seed des données** : `seed_db()` ne s'exécute que si la base est vide. Pour modifier le seed, il faut supprimer `data/atelier.db`.
...
- **Debounce position/taille** (`blocsStore.ts`) : délai de 300ms avant persistance. Si la page se ferme pendant le debounce, la dernière position est perdue.
- **Catch silencieux** : plusieurs composants React (ConsoleIA, espaceStore, blocsStore, ConfigIA) ont des blocs `catch(() => {})` ou `catch(e => setError(...))` qui ne loguent pas l'erreur — debug difficile en cas de panne API.
- **Routeur IA format unique** : `_call_api()` assume un format OpenAI-compatible (messages[]) pour toute API externe. Si un provider utilise un format différent, ça échouera silencieusement (catch → null).
- **Ollama vs API** : le mode local utilise `/api/generate` (format Ollama natif, champ `prompt`) tandis que le mode API utilise `/chat/completions` (format OpenAI, champ `messages`). Deux formats incompatibles gérés séparément.
- **Timeout IA** : 30s pour les appels IA, 10s pour les tests de connexion. Sur des modèles lents ou des connexions faibles, risque de timeout avant réponse.
- **Canvas hit-testing** : le hit-testing des formes (cloud, oval) utilise des approximations géométriques. Décalage possible entre la zone visuelle et la zone cliquable sur les formes complexes.
...
- **`iaStore.ts` inutilisé** : fichier vide (une seule ligne de commentaire), jamais importé. Le state IA est géré directement dans les composants.
- **`ia_graphe.py` vide** : service squelette (une ligne de commentaire). L'indexation est gérée par `indexation.py` mais `ia_graphe.py` n'est jamais appelé.
- **Services backend squelettes vides** : `vocal.py`, `embeddings.py`, `import_parser.py`, `recherche_externe.py`, `meta_graphe.py`, `generateur_graphe.py` — fichiers réservés pour fonctionnalités futures, contiennent uniquement un commentaire.
- **Champ `titre` absent du schéma** : l'interface `BlocAPI` côté frontend définit `titre` mais le schéma SQLite n'a que `titre_ia` (généré par l'IA). Le frontend utilise `titre_ia || titre || ''` comme fallback.
- **Styles dupliqués** : les composants ConfigIA, ConsoleIA et BlocEditor partagent des patterns CSS similaires (glassmorphism, overlay) sans factorisation.
...
**Les flux événementiels (implémentés) :**
...
- Déplacement bloc → événement `bloc:move` → debounce 300ms → PUT API → Backend persiste
- Redimensionnement bloc → événement `bloc:resize` → debounce 300ms → PUT API → Backend persiste

**Points d'entrée :**

- **Lancement** : `lancer.bat` — démarre backend (fenêtre minimisée) puis frontend avec ouverture navigateur
- **Frontend** : `frontend/src/main.tsx` → `App.tsx`
- **Backend** : `backend/main.py` → FastAPI app avec lifespan (init_db/seed_db/close_db)
...
- `backend/main.py` : Point d'entrée — lifespan (init_db, seed_db, close_db), CORS, routes, chargement .env
- `backend/db/schema.sql` : Schéma SQLite — 5 tables (espaces, blocs, contenus_bloc, liaisons, config_ia) + index
- `backend/db/database.py` : Connexion SQLite (aiosqlite), init_db, close_db, get_db, seed_db
- `backend/api/espaces.py` : CRUD espaces — POST/GET/PUT/DELETE avec cascade
- `backend/api/blocs.py` : CRUD blocs — POST/GET/PUT/DELETE, validation forme/couleur, gestion contenus
- `backend/api/liaisons.py` : CRUD liaisons — POST/GET/DELETE, vérification FK, auto-liaison interdite
- `backend/api/config_ia.py` : Config IA — GET/PUT par rôle + POST test connexion
- `backend/services/ia_routeur.py` : Dispatch IA — Ollama (local) ou API OpenAI-compatible (OpenRouter)
- `backend/services/ia_assistant.py` : Assistant IA — protocole, heuristiques R1-R5/A1-A5/C1-C5, build_context
- `backend/services/indexation.py` : Indexation auto — titre_ia, resume_ia, entités, mots-clés via IA Graphe

**Frontend — Canvas2D :**

- `frontend/src/canvas/engine.ts` : Moteur de rendu — boucle 60 FPS, pan/zoom, coordonnées monde/écran
- `frontend/src/canvas/shapes.ts` : 5 formes (cloud, rounded-rect, square, oval, circle), glow, gradient 3D, reflet
- `frontend/src/canvas/links.ts` : 4 types liaisons Bézier cubiques, tangence réelle, glow lumineux
- `frontend/src/canvas/interactions.ts` : Drag, zoom, sélection, hit-testing, création liaison, redimensionnement
- `frontend/src/canvas/legends.ts` : Légendes contextuelles — matrice couleur×forme (30 entrées), fade auto
- `frontend/src/canvas/theme.ts` : Paramètres visuels externalisés — couleurs, tailles, effets
- `frontend/src/canvas/events.ts` : Bus d'événements Canvas ↔ React (pivot architectural)

**Frontend — Composants React :**

- `frontend/src/App.tsx` : Composant racine — layout, intégration Canvas + tous les panneaux
- `frontend/src/components/TopBar.tsx` : Sélecteur d'espace, création, titre central
- `frontend/src/components/SidePanel.tsx` : Explorateur — vue liste triable, recherche, sélection → zoom
- `frontend/src/components/BottomBar.tsx` : Commandes — sauver, recentrer, zoom, toggle IA, config, stop
- `frontend/src/components/ConsoleIA.tsx` : Panneau dialogue IA — historique, saisie, translucide
- `frontend/src/components/ConfigIA.tsx` : Config IA — 2 rôles, mode/URL/modèle/clé, save + test
- `frontend/src/components/BlocEditor.tsx` : Éditeur de bloc — texte, URL, fichiers, drag&drop, paste

**Frontend — Stores et API :**

- `frontend/src/stores/espaceStore.ts` : Hook useEspaceStore — chargement, création, sélection d'espaces
- `frontend/src/stores/blocsStore.ts` : Hook useBlocsStore — CRUD synchronisé, toBlocVisuel, toLiaisonVisuelle, debounce
- `frontend/src/api.ts` : Client REST typé — espaces, blocs, liaisons, contenus, config IA, assistant
- `frontend/vite.config.ts` : Proxy API `/api` → backend:8000, `/ws` → WebSocket
...
- `init_db()` (`database.py`) : Initialise la connexion SQLite et exécute le schéma. Si elle échoue, rien ne fonctionne.
- `close_db()` (`database.py`) : Ferme proprement la connexion. Sans elle, risque de corruption.
- `get_db()` (`database.py`) : Retourne la connexion active. Utilisée par toutes les routes.
- `seed_db()` (`database.py`) : Peuple les données initiales (2 espaces, 6 blocs, contenus, liaisons, config IA depuis .env). Ne s'exécute que si la base est vide.
- `lifespan()` (`main.py`) : Cycle de vie FastAPI — init_db → seed_db au démarrage, close_db à l'arrêt.

**Backend — Services IA :**

- `call_ia(role, prompt, system)` (`ia_routeur.py`) : Point d'entrée unique pour appeler l'IA. Dispatche vers Ollama ou API selon la config du rôle. Retourne None si non configuré ou erreur.
- `_call_ollama(url, modele, prompt, system)` (`ia_routeur.py`) : Appel Ollama natif `/api/generate`. Format prompt unique.
- `_call_api(url, modele, cle_api, prompt, system)` (`ia_routeur.py`) : Appel API OpenAI-compatible `/chat/completions`. Headers OpenRouter inclus.
- `ask_assistant(espace_id, question)` (`ia_assistant.py`) : Dialogue IA — construit le contexte de l'espace, envoie au modèle avec le system prompt (heuristiques incluses).
- `build_context(espace_id)` (`ia_assistant.py`) : Sérialise un espace complet (blocs, contenus, liaisons) en texte pour le contexte IA.
- `indexer_bloc(bloc_id)` (`indexation.py`) : Génère titre_ia, resume_ia, entités, mots-clés via IA Graphe. Déclenchée à la création/modification de contenu.
- `test_connection()` (`config_ia.py`) : Teste la connexion IA par rôle — Ollama `/api/tags` ou API `/models`, timeout 10s.

**Frontend — Canvas2D :**

- `CanvasEngine` class (`engine.ts`) : Moteur principal — init, boucle render, gestion pan/zoom/DPR, conversion coordonnées monde↔écran.
- `drawShape()` (`shapes.ts`) : Dessine un bloc selon forme+couleur avec tous les effets visuels (gradient, glow, reflet, connecteurs).
- `drawLink()` (`links.ts`) : Dessine une liaison Bézier cubique entre deux blocs avec tangence, glow et couleur sémantique.
- `LegendManager.draw()` (`legends.ts`) : Affiche la légende contextuelle 3 lignes (matrice, couleur, forme) avec fade automatique.

**Frontend — Stores :**

- `toBlocVisuel(bloc)` (`blocsStore.ts`) : Convertit un bloc API en BlocVisuel pour le Canvas. Utilise `titre_ia || titre || ''`.
- `toLiaisonVisuelle(liaison)` (`blocsStore.ts`) : Convertit une liaison API en LiaisonVisuelle pour le Canvas.
- `useBlocsStore()` (`blocsStore.ts`) : Hook — charge blocs+liaisons par espace, CRUD synchronisé, debounce move/resize 300ms.
- `useEspaceStore()` (`espaceStore.ts`) : Hook — chargement auto des espaces, création, sélection.
